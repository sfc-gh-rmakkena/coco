...
with tab1:
    ...
    if not df_won.empty:
        ...
        st.caption(f"Showing {len(df_won)} won+ engagements | Select rows using checkboxes for individual AI analysis")
        
        selected_indices = edited_df[edited_df['Select'] == True].index.tolist()
        selected_rows = df_won.iloc[selected_indices] if selected_indices else pd.DataFrame()
    else:
...
with tab2:
    ...
    if not df_all.empty:
        ...
        st.caption(f"Showing {len(df_all)} engagements | Select rows using checkboxes for individual AI analysis")
        
        selected_indices_all = edited_df_all[edited_df_all['Select'] == True].index.tolist()
        selected_rows_all = df_all.iloc[selected_indices_all] if selected_indices_all else pd.DataFrame()
    else:
